...
class LidarVFHController(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('view_angle_deg', 150.0)
        self.declare_parameter('min_range', 0.2)
        self.declare_parameter('max_range', 6.0)
        self.declare_parameter('inflation_max_range', 3.0)
    ...
    def scan_callback(self, msg: LaserScan) -> None:
        ...
        if ranges.size == 0:
            self.latest_scan_dist = None
            return

        angle_array = msg.angle_min + np.arange(ranges.size, dtype=np.float32) * msg.angle_increment

        valid = np.isfinite(ranges)
        valid &= ranges > max(self.min_range, float(msg.range_min))
        valid &= ranges < min(self.max_range, float(msg.range_max))

        in_fov = (angle_array >= -self.view_angle) & (angle_array <= self.view_angle)
        valid &= in_fov

        if not np.any(valid):
            self.latest_scan_dist = np.full(self.num_sectors, np.inf, dtype=np.float32)
            return

        angles = angle_array[valid]
        dist = ranges[valid]
        ...
        np.minimum.at(sector_dist, indices, dist)
        self.latest_scan_dist = sector_dist
    ...
    def control_loop(self) -> None:
        ...
        blocked = self.compute_blocked_sectors(self.latest_scan_dist)
        selected_heading, selected_dist = self.select_heading(goal_heading_robot, self.latest_scan_dist, blocked)
        ...
        if np.all(blocked):
            ...
            cmd.angular.z = math.copysign(
                ...
                goal_heading_robot if abs(goal_heading_robot) > 1e-4 else 1.0,
            )
            self.cmd_pub.publish(cmd)
            return

        cmd.angular.z = self.clamp(
            self.heading_kp * selected_heading,
    ...
    def select_heading(
        ...
        clearance[~np.isfinite(clearance)] = self.max_range
        clearance = np.clip(clearance / self.max_range, 0.0, 1.0)
